...
In this blog have details the configuration and how to deploy the Minio with Certificate manager in Kind cluster. The Apisix deployed will act as an Gateway so the Minio tenant console can be accessed from the host machine.

Only the configuration and deployment instruction is explained here for concepts please refer the documentation.
...
#### Pre-requsities 
 ...
 #### Summary
  - The kind cluster will be deployed in the Docker, the Cert-manager will deployed using the kuberentes manifest using `kubectl apply`. The Apisix and Minio are deployed using Helm charts. To access the Minio and Apisix Dashboard from host machine we use https url the certifcate are self-signed certificates. Routing configuration is applied using Apisix Ingress controller.
...
Deploy the cerfificate to the minio-operator using below command.
...
**Note:** 
 - Download the helm chart to local using `helm pull minio-opertor/tenant --untar`. Copy the default values yaml file to local and update that for further tenant deployment.
